...
// === Native Audio Understanding (mimo-v2.5) — bypasses ASR for call mode ===
async function callMiMoWithAudio(audioBase64, options) {
  const { system, history, max_tokens = 650 } = options;
  const apiMessages = [];

  if (system) apiMessages.push({ role: "system", content: system });

  // Add conversation history (text messages)
  ...
  // Add current user message with native audio input (multimodal)
  apiMessages.push({
    role: "user",
    content: [
      {
      ...
      {
        type: "text",
        text: "请根据用户的语音消息，按照系统提示的JSON格式回复。"
      }
    ]
  });
  ...
  if (!res.ok) {
    const errText = await res.text().catch(() => "");
    throw new Error("MiMo Audio API 错误 (" + res.status + "): " + (errText || "").slice(0, 200));
  }
  ...
  if (!content) {
    if (msg?.reasoning_content) {
      console.warn("Audio model returned reasoning only:", msg.reasoning_content.slice(0, 300));
      throw new Error("模型只返回了推理过程，未生成回复");
    }
    console.warn("Audio API response:", JSON.stringify(data).slice(0, 1000));
    throw new Error("API 返回无内容");
  }
  return content;
}
