...
async function sendDressupOutfit() {
  ...
  row.innerHTML = `
    <div class="bubble user">
      <div style="font-size:11px;color:rgba(255,255,255,0.6);margin-bottom:6px">👔 递给Ta一件衣服</div>
  ...
  chatMessages.push({ role: "user", text: `[换装:递给Ta一件衣服]${extraText ? " " + extraText : ""}`, isImage: true, imgId, time: ts });
  saveChatHistory();

  // Send to MiMo — normal image message with dressing context
  const dressupMsg = extraText
    ? `[用户打开了更衣间，递给你一张衣服的图片想让你换上，同时说: "${extraText}"] 看看图片里的衣服，根据你的性格和喜好自然地反应——你可以欣然接受穿上、嫌弃但勉强穿、不想穿、吐槽衣服风格、或者任何符合你性格的反应。记住你是有自己审美和主见的${characterProfile.botRole || "boyfriend"}。IMPORTANT: Reply with 2-4 separate JSON messages!`
    : `[用户打开了更衣间，递给你一张衣服的图片想让你换上] 看看图片里的衣服，根据你的性格和喜好自然地反应。记住你是有自己审美和主见的${characterProfile.botRole || "boyfriend"}。IMPORTANT: Reply with 2-4 separate JSON messages!`;
...
async function sendDefaultOutfit() {
  ...
  row.innerHTML = `
    <div class="bubble user" style="text-align:center;font-size:14px">
      ...
      <span style="color:var(--user-bubble-text)">让Ta换回默认穿搭</span>
    </div>
    <div class="msg-time">${formatMsgTime(ts)}</div>`;
  area.appendChild(row);
  area.scrollTop = area.scrollHeight;

  chatMessages.push({ role: "user", text: "[换装:让Ta换回默认穿搭]", time: ts });
  saveChatHistory();

  const defaultMsg = `[用户打开更衣间，让你换回你的日常穿搭] 自然地反应，可以开心地换回来、或者假装不舍得刚才那件衣服。IMPORTANT: Reply with 1-3 separate JSON messages!`;

  const systemPrompt = await prepareBotContext("换回默认衣服", "[换装:换回默认穿搭]", "[让Ta换回默认穿搭]");
  ...
  try {
    ...
    setLoading(false);
    appendBotMessage("[Changed back to default outfit]", "换回来了～还是这身舒服呢", null, false);
  }
